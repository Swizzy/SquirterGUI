...
namespace SquirterGUI
{
    ...
    public class FTDI
    {
        ...
        public FtStatus Read(byte[] dataBuffer, UInt32 numBytesToRead, ref UInt32 numBytesRead)
        {
            // Initialise ftStatus to something other than FT_OK
            var ftStatus = FtStatus.FtOtherError;
            ...
            if (_pFTRead != IntPtr.Zero)
            {
                ...
                {
                    // Call FT_Read
                    ftStatus = ftRead(_ftHandle, dataBuffer, numBytesToRead, ref numBytesRead);
                }
            }
            else
            {
